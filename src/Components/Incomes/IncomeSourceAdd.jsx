...
const IncomeSourceAdd = props => {
    ...
    React.useEffect(() => {
        ...
        if (save) {
            ...
            axios.put('incomes/source/save', { ...formdata, toPays })
                .then(({ data }) => {
                    ...
                    typeof setRows == "function" && setRows(p => {
                        ...
                        parts.map((p, k) => {
                            ...
                            p.rows.map((r, i) => {
                                if (r.id === dataRow?.id) {
                                    parts[k].rows[i] = dataRow;
                                    add = false;
                                }
                            });
                            if (add && p.id === dataRow?.part_id) parts[k].rows.unshift(dataRow);
                        });
                        return parts;
                    });
    ...
    return <Modal
        ...
        content={<div className="content position-relative">
            ...
            <Form>
                ...
                <Form.Group widths={2} className="align-items-center pt-1">
